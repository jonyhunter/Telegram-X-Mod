...
public class TgxCacheDocumentsProvider extends DocumentsProvider {
  ...
  public void deleteDocument (String documentId) throws FileNotFoundException {
    ...
    if (!deleteRecursive(file)) {
      Log.w("Failed to delete cache document: %s, path: %s", documentId, file.getAbsolutePath());
  ...
  public void removeDocument (String documentId, String parentDocumentId) throws FileNotFoundException {
    if (!isChildDocument(parentDocumentId, documentId)) {
      throw new FileNotFoundException("Document is not a child of parent: " + documentId);
    }
  ...
  private boolean deleteRecursive (File file) {
      ...
      if (children != null) {
        for (File child : children) {
          if (!deleteRecursive(child)) {
            Log.w("Failed to delete child cache document: %s", child.getAbsolutePath());
            return false;
          }
        }
      }
    ...
    if (!deleted && file.exists()) {
      Log.w("Failed to delete cache file: %s, canWrite: %b, isDirectory: %b", file.getAbsolutePath(), file.canWrite(), file.isDirectory());
    }
